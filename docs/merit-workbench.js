...
const ADDRESS='0x48B8995384E8340892554567965aA5910c69B380', ENDPOINT='https://studio.genlayer.com/api';
let wallet,account;
...
<header class="mast"><div class="serial">MERIT OFFICE<br>FORM 08</div><div><h1>Contribution<br>review sheet</h1></div><div class="badge">NO EPOCH LOADED</div></header>
...
<div class="step"><div class="step-no">02</div><div><div class="formula"><div class="factor">QUALITY EVIDENCE<b>60</b></div><div class="factor">ADOPTION EVIDENCE<b>40</b></div></div><div class="fields"><label>Quality source URL<input id="source1" placeholder="https://..."></label><label>Adoption source URL<input id="source2" placeholder="https://..."></label><label class="wide">Requested earliest finalization<input id="deadline" type="datetime-local"></label></div><p>Choose two source URLs. Scoring always grants at least 10 minutes (StudioNet demo minimum) to appeal, even after the requested date.</p></div></div>
...
<div class="ledger"><b>REVIEW LEDGER</b><div class="ledger-row"><span>Stored source slots</span><span id="source-count">--</span></div><div class="ledger-row"><span>Weight equation</span><span>100</span></div><div class="ledger-row"><span>Appeal window</span><span id="appeal-status">Not scored</span></div></div>
...
async function load(){const result=await reader.readContract({address:ADDRESS,functionName:'get_epoch',args:[value('id')]});show(result);q('#source-count').textContent=result.sources.length;q('#score').textContent=result.state==='OPEN'?'--':result.score;q('#appeal-status').textContent=result.state==='OPEN'?'Not started':new Date(Number(result.appealDeadline)*1000).toLocaleString();q('.badge').textContent=result.state;return result}
